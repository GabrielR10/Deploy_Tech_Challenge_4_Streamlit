...
#st.write(f"Versão do Python: {sys.version}") #Versão do Python: 3.10.11 (tags/v3.10.11:7d4cc5a, Apr 5 2023, 00:38:17) [MSC v.1929 64 bit (AMD64)]


#Teste de importação das bibliotecas = SEM ERRO
# try:
#     from streamlit_option_menu import option_menu
# except ImportError as e:
#     import streamlit as st
#     st.error(f"Erro ao importar streamlit_option_menu: {e}")
...
df_tabela_previsao = pd.read_csv("dataset-previsao-editado4.csv")
df_grafico_previsao = pd.read_csv("Previsao editado.csv")
...
if selected == "Página Inicial":
    ...
    st.image("20220316090049_barril-petr-leo.jpg")
    ...
    texto1 = """
        ...
        Na Fase 4 tivemos disciplinas que abordaram os temas de:
        - **Análise de Negócios**;
    ...
    texto2 = """ 
        ...
        - E também elaboramos a criação de um modelo de Machine Learning que realiza a previsão do preço do petróleo diariamente. 
        """
    st.write(texto2)
    st.markdown('---')
    texto3 = """
    ...
    def calcula_valor_medio(df_novo_completo):
    ...
    with tab1:
        ...
        st.write("""
            ...
            90 dólares, passando para 134 com uma alta de mais de 40% e depois seguido por uma queda para 40 dólares, uma queda incrível de quase 75%.
                 """)

        # Criar o gráfico de linhas com Plotly Express
        fig = px.line(df_2008_resampled_monthly, x='Data', y='preco', title='Variação do preço do petróleo Brent em 2008')

        # Adicionar rótulos aos pontos
        fig.update_traces(
            mode='markers+lines+text', 
            text=df_2008_resampled_monthly['preco'].round(2).astype(str),  # Certifique-se de que o texto seja string
    ...
    with tab2:
        ...
        st.write("""
            ...
            o aumento esperado no consumo. Como tinha muito petróleo sendo produzido e pouco sendo consumido, os preços acabaram caindo quase 50% em menos de 6 meses.
                 """)
        # Criar o gráfico de linhas com Plotly Express
        fig = px.line(df_2014_resampled_monthly, x='Data', y='preco', title='Variação do preço do petróleo Brent em 2014')

        # Adicionar rótulos aos pontos
        fig.update_traces(
            mode='markers+lines+text', 
            text=df_2014_resampled_monthly['preco'].round(2).astype(str),  # Certifique-se de que o texto seja string
            textposition='top center'
        )
    ...
    with tab3:
        ...
        st.write( """
            Em 2020 tivemos uma das piores pandemias da história, a pandemia do Coronavírus. A incerteza no cenário mundial afetou diretamente o preço de praticamente todos os ativos
            do mundo e com o petróleo não foi diferente, chegando a bater 18.47 dólares em maio, um dos valores mais baixos da série histórica.
                 """)
            # Criar o gráfico de linhas com Plotly Express
        fig = px.line(df_2020_resampled_monthly, x='Data', y='preco', title='Variação do preço do petróleo Brent em 2020')

        # Adicionar rótulos aos pontos
        fig.update_traces(
            mode='markers+lines+text', 
            text=df_2020_resampled_monthly['preco'].round(2).astype(str),  # Certifique-se de que o texto seja string
            textposition='top center'
        )
    ...
    with tab4:
        ...
        st.write("""
            ...
            Essas sansões dificultavam a exportação do petróleo russo, gerando assim uma incerteza no mercado e a grande alta, chegando a atingir valores acima de 122 dólares, patamar não alcançado desde 2014.
                 """)
              # Criar o gráfico de linhas com Plotly Express
        fig = px.line(df_2022_resampled_monthly, x='Data', y='preco', title='Variação do preço do petróleo Brent em 2022')

        # Adicionar rótulos aos pontos
        fig.update_traces(
            mode='markers+lines+text', 
            text=df_2022_resampled_monthly['preco'].round(2).astype(str),  # Certifique-se de que o texto seja string
            textposition='top center'
        )
    ...
    st.title("Previsões para os próximos 60 dias 🔮")
    st.write("Aqui mostramos as previsões de fechamento do barril de petróleo nos próximos dias.")
    ...
    with col[0]:
        ...
        st.dataframe(df_tabela_previsao)

    with col[1]:
        st.markdown('#### Gráfico de Tendência dos Valores Previstos')

         # Criar o gráfico de linhas com Plotly Express
        fig = px.line(df_grafico_previsao , x='Data', y='Valor', title='Variação do preço do petróleo previsto')
    ...
    st.markdown("---")
    st.subheader("Informações sobre o modelo de Machine Learning utilizado")
    texto_modelo_ML = """
        Utilizamos as bases de dados do site do IPEA segmentando para trazer os dados do século atual (04/01/2000 até 13/05/2024). \n
        Após realizar a análise da série histórica notamos alguns outliers em certos períodos históricos e após vários testes notamos que os dados da pandemia estavam mais atrapalhando do que ajudando, pois foi algo muito incomum que aconteceu e inferir isso no futuro acabava gerando mais erros. \n
        Utilizamos os dados à partir de 2021 e fizemos o teste de 3 modelos diferentes de ML **(ARIMA, MÉDIA MÓVEL e PROPHET)**, podemos dizer que o modelo que obtivemos uma melhor performance foi o **Prophet**.  \n
        O Prophet é um modelo de machine learning desenvolvido pelo Facebook para fazer previsões de séries temporais, que são dados ordenados cronologicamente, como vendas ao longo dos meses ou a temperatura diária.\n
        Aqui no nosso desafio serviu perfeitamente e além disso realizamos alguns ajustes como a criação de um df contendo os dias no brasil que não possui fechamento de bolsa (dias não úteis), para posteriormente excluir essas datas da predição.\n
        """
    st.write(texto_modelo_ML)
    st.write("Para saber mais detalhes do código, detalhamos o passo a passo no link abaixo: 👇")
    st.write("https://colab.research.google.com/drive/1VCiQqKy1Qb3ojil4FYDVoNdeGihbvEGH#scrollTo=RrxbW2teU5kO")
    ...
    texto_sobre = """
        ...
        Tivemos o desafio em atender a essa demanda realizando uma análise com grandes conjuntos de dados obtidos pelo site do IPEA (Instituto de Pesquisa Econômica Aplicada) gerando insights relevantes e auxiliando para a tomada de decisão do nosso cliente.
        \n
        \n
